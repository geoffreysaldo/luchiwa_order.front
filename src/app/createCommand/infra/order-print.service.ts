...
export class OrderPrintService {
  ...
  buildTicket(order) {
    console.log(order)
    this.order = order;
    this.doc = new jsPDF('p', 'mm', [170, 210]);
    this.setPdfConfig();
    this.setHeader();
  ...
  setHeader() {
    this.doc.text("L'UZUMAKI",95,this.y);
    this.doc.setFontSize(14);
    this.y = this.y + 6;
    this.doc.text("ZAC LES BARLES", 83,this.y);
    this.y = this.y + 6;
    this.doc.text("13470 CARNOUX EN PROVENCE", 50, this.y);
    this.y = this.y + 6;
  ...
  setProductsLines() {
    ...
    this.order.products.map((item,index) => {
      ...
      this.doc.text((item.product.price*item.quantity).toFixed(2),113,this.y);
      this.y = this.y + 6;
    })
    this.y = this.y - 1
  ...
  setTotals() {
    ...
    if(this.order.discount){
      ...
      this.doc.text(this.order.discount.toString() + "%", 113, this.y);
    }
    this.y = this.y + 7;
    this.doc.text("Total HT",10,this.y);
    this.doc.text(this.order.totalHT.toString(), 113, this.y);
    this.y = this.y + 7;
    this.doc.text("Total TVA",10,this.y);
    this.doc.text((this.order.total - this.order.totalHT).toFixed(2), 113, this.y);
    this.y = this.y + 7;
    this.doc.setFontSize(15);
    this.doc.text("Total TTC (Eur)",10,this.y);
    this.doc.text(this.order.total.toFixed(2), 113, this.y);
    this.doc.setFontSize(14);
    this.y = this.y + 4;
    this.doc.text("========================================", 10, this.y);
  }
  ...
  setTaxes() {
    ...
    this.doc.text("CA HT Eur",57,this.y);
    this.doc.text("Mnt TVA Eur",98,this.y);
    this.y = this.y + 6
    ...
    taxRateArray.map((item,index) => {
      ...
      this.doc.text((totalHT[index] - (totalHT[index] * this.order.discount)/100).toFixed(2),61,this.y);
      this.doc.text((totalTax[index] - (totalTax[index] * this.order.discount)/100).toFixed(2),112,this.y);
      this.y = this.y + 6;
    })
  }
  ...
  save() {
    ...
    this.doc.text("Siret: 8244848855", 87, this.y);
    this.y = this.y + 7
    this.doc.setFontSize(15);

    this.doc.text("Merci, à bientôt", 87, this.y);
